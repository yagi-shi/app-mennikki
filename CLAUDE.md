...
### UI/UX改善
- [ ] 新規記録画面の必須項目に必須マーク（`*`）を表示する
- [ ] DatePickerの日本語ロケール対応（カレンダー・曜日の日本語表示）
- [ ] 詳細画面から写真のみを編集（差し替え・削除）できるようにする
- [ ] ラーメン種類フィルターを登録済みデータのみに絞り込む（都道府県フィルターと同様の挙動）
- [ ] フィルター適用中であることを視覚的に示す（バッジやボタン色の変化など）
- [ ] 一覧→詳細→一覧に戻った際、スクロール位置によって検索バーの表示がちらつく問題の改善
